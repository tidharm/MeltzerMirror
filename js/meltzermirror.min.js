...
/** To reset localization
	Paste the following line in the browser and run it, then reload the page:
	javascript:localStorage.removeItem("localization");
**/
...
if (localization == "he"){
	...
	var dayShortNames = ['א', 'ב', 'ג', 'ד', 'ה', 'ו', 'ש'];
	var weatherDescriptions = {"clear sky": "בהיר", "few clouds": "מעונן חלקית", "scattered clouds": "מעונן", "broken clouds": "עננות כבדה", "shower rain": "גשום", "rain": "גשם מקומי", "thunderstorm": "סופות רעמים", "snow": "שלג", "mist": "ערפל"};
	var fcCellOrder = ['fc-mntp','fc-mxtp','fc-icon','fc-wday'];
	var compliments = ['...להחזיק מעמד','!יאללה לעבודה','אין מה לראות כאן, נא להתקדם','בלה בלה בלה','אולי עדיף להישאר בבית היום...'];
}else{
	// English (Default)
	var monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
	var dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
	var dayShortNames = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
	var weatherDescriptions = {"clear sky": "Clear Sky", "few clouds": "Few Clouds", "scattered clouds": "Scattered Clouds", "broken clouds": "Broken Clouds", "shower rain": "Shower Rain", "rain": "Rain", "thunderstorm": "Thunderstorm", "snow": "Snow", "mist": "Mist"};
	var fcCellOrder = ['fc-wday','fc-icon','fc-mntp','fc-mxtp'];
	var compliments = ['Hang in there...','Keep working dude','Nothing to see here','Bla Bla Bla','Maybe it\'s better to stay home today...'];
}
...
function getWeekday(tsUnix, shortName) {
  ...
  //weekDay = dayNames[date.getDay()].substring(0, (shortName ? 3 : 9)); // if shortName==true, take only 1st 3 letters
  weekDay = (shortName ? dayShortNames[date.getDay()] : dayNames[date.getDay()]);
  return weekDay;
}
...
// Google Maps API
var googleMapsUrl = "https://maps.googleapis.com/maps/api/geocode/json?key="+googleMapsApiKey+"&sensor=true&language="+localization;
...
sendRequest('GET', ipInfoUrl).then(
  function (reqSuccess) {
    ...
    if (ipInfo.loc) {
	  ...
	  googleMapsUrl = googleMapsUrl + "&latlng="+locLat+","+locLon;
    } else if (ipInfo.city) {
	  ...
	  googleMapsUrl = googleMapsUrl + "&address="+ipInfo.city;
    } else {
	  ...
	  googleMapsUrl = googleMapsUrl + "&address=Eilat"; // Default, so you'll know something is wrong
    }
    ...
    sendRequest('GET', weatherUrl).then(
      function (reqSuccess) {
		...
		sendRequest('GET', googleMapsUrl).then(
		  function (reqSuccess) {
			if (logLevel > 3) {console.log('[Trace] ' + "Google Maps Geocode result: " + reqSuccess);}
			// Parse response
			var gJsonRes = JSON.parse(reqSuccess);
			
			
			var gCityName = "Eilat"; // Default, so you'll know something is wrong
			if (gJsonRes.plus_code){
			  var compoundCode = gJsonRes.plus_code.compound_code;
			  gCityName = compoundCode.substring(compoundCode.indexOf(" ")+1);
			} else {
			  gCityName = gJsonRes.results[0].formatted_address;
			}
			
			var wDesc = wJsonRes.weather[0].description;
			//var weatherDescription = capitalizeTitle(wDesc);
			var weatherDescription = weatherDescriptions[wDesc];
            ...
            document.getElementById('weatherCity').innerHTML = gCityName; //wJsonRes.name;
            ...
            setTimeout(function() {
              sendRequest('GET', forecastUrl).then(
                function (reqSuccess) {
                  ...
                  //Update webpage
				  var fcTable = document.getElementById("fc-table");
				  
                  for (var r = 0; r < curForecast.length; r++) {
					//Create table rows
					var fcRow = document.createElement("tr");
					fcRow.id = "fc-row-day"+r;
					
					//Create row cells
					for (var d = 0; d < fcCellOrder.length; d++) {
					  var fcCell = document.createElement("td");
					  var cellName = fcCellOrder[d];
					  fcCell.classList.add(cellName);
					  if (cellName.includes("icon")){
					    //Add img element
						var fcImg = document.createElement("img");
						fcImg.id = cellName+r;
						fcImg.alt = "";
						fcCell.appendChild(fcImg);
					  } else {
					    fcCell.id = cellName+r;
					  }
					  fcRow.appendChild(fcCell);
					}
					//Add row to table
					fcTable.appendChild(fcRow);
					
					//Update row values
                    document.getElementById('fc-wday'+r).innerHTML = curForecast[r][0];
                    document.getElementById('fc-icon'+r).src = 'images/50px/' + curForecast[r][1] + '.png';
                    document.getElementById('fc-mntp'+r).innerHTML = Math.round(curForecast[r][2]) + degChar;
                    document.getElementById('fc-mxtp'+r).innerHTML = Math.round(curForecast[r][3]) + degChar;
                  }
                  //==================================================//
            ...
            if (logLevel > 0) {console.error("Google Maps failed: " + reqFail);}
		  }
		);
      }, function (reqFail) {
